...
art = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.playiptv/art', ''))
...
THUMBNAIL = "thumbnail"
TV_SHOWS = "tvshows"
wachusername = plugintools.get_setting("playiptvusername")
wachpass = plugintools.get_setting("playiptvpass")
morros = plugintools.get_setting("morros")
url = "http://www.playiptvpr.com/wp/wp-login.php?=" + playiptvusername + "&password=" + playiptvpass + "&type=get_live_categories"
yea = "http://www.playiptvpr.com/wp/wp-login.php?" + playiptvusername + "&password=" + playiptvpass 
chifu = "&type=get_vod_streams&cat_id="



def run():
    plugintools.log("wplayiptv.run")
...
def xtv(params):
  ...
  if xtvmaster == 'false':
   plugintools.message("PLAYIPTV","ACCEDE PRIMERO AL PORTAL")
   plugintools.open_settings_dialog()
        ...
        if "<items>" in data:
           ...
           xbmc.executebuiltin("Notification(%s,%s,%s,%s)" % ('PLAYIPTV', "USUARIO O CONTRASEÑA INCORRECTA......", 10 , art+'b.png')) 
           plugintools.open_settings_dialog()
...
def main_list(params):
	...
	plugintools.add_item( 
        action = "perro", 
        title = "SOLO ADULTOS ",
        url = "http://149.202.202.127:8000/enigma2.php?username=" + playiptvusername + "&password=" + playiptvpass + "&type=get_live_streams&cat_id=" + "41",
        thumbnail = nopor,
        fanart = estati,
        folder = True )
...
def parse2(params):
    ...
    plugintools.add_item( 
        action = "perro", 
        title = "MUSICA VIDEOS",
        url = "http://www.playiptvpr.com/wp/wp-login.php?" + wachusername + "&password=" + wachpass + "&type=get_live_streams&cat_id=19",
        thumbnail = music,
        folder = True )
    plugintools.add_item(
        action = "search5",
...
def perro3(params):
    ...
    for entry in SongLists:
            ...
            titulo = base64.b64decode(titulo)
            titulo = titulo.upper()
            titulo = titulo.replace("PARENT-CODE=\"1234\",","")
            description = plugintools.find_single_match(entry, '<description>([^"]+)</description>')
            description = base64.b64decode(description)
            playstation = plugintools.find_single_match(entry, '<playlist_url>([^"]+)</playlist_url>')
            playstation = playstation.replace("<![CDATA[", "")
...
def cuenta(params):
    url_getlink = 'http://www.playiptvpr.com/wp/wp-login.php?' + playiptvusername + "&password=" + playiptvpass
    request_headers=[]
    ...
    for entry in datos:
     ...
     selector = dialog.select('PLAYIPTV', ['# VER USERNAME Y PASSWORD' , '# VERIFICAR STATUS Y FECHA DE VENCIMIENTO' ])
    if selector == 0:
        plugintools.message("PLAYIPTV INFORMA" , "USERNAME:" + ima , "PASSWORD:" + ma  )
    if selector == 1:
        plugintools.message("PLAYIPTV INFORMA" , "STATUS:" + esta , "FECHA DE VENCIMIENTO:" + ven )
...
def perro4(params):
    ...
    for entry in SongLists:
            ...
            titulo2 = titulo2.replace("PARENT-CODE=\"1234\",","")
...
def search5(params):
    dialog = xbmcgui.Dialog()
    selector = dialog.select('PLAYIPTV', ['BUSQUEDA POR CANCION', 'BUSQUEDA POR ARTISTA'])
    if selector == 0:
        ...
        if "id" in data:
           ...
           for entry in songs:
            ...
            plugintools.add_item(action="play2", title = title_song + " " +'[COLOR purple] ('+songtime+')[/COLOR]', url=url, thumbnail = thumbnail , fanart = "" , folder = False, isPlayable = True)
        elif "0" in data:
             errormsg = plugintools.message("PLAYIPTV","SIN RESULTADOS")
             search5(params)
    if selector == 1:
        texto = plugintools.keyboard_input()
        plugintools.set_setting("last_search",texto)
        ...
        if "id" in data:
           ...
           for entry in songs:
            ...
            songtime = plugintools.find_single_match(entry, '"songtime":"([^"]+)')
            plugintools.add_item(action="songs2", title = title_song + " " + '[COLOR red]('+ plsongs +')[/COLOR]' + 'ITEMS' + '[COLOR purple] ('+songtime+')[/COLOR]' +  'DURACION'  , url = url , thumbnail = lista , fanart = ""  , folder = True, isPlayable = False)
            i = i + 1			
        elif "0" in data:
             errormsg = plugintools.message("PLAYIPTV","SIN RESULTADOS")
             search5(params)		
